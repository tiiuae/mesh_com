...
function server {
  ...
  /opt/mesh_com/script/generate_keys.sh /etc/ssl/certs/
  KEY_PATH="src/root_key.der"
  cp /etc/ssl/certs/mesh_key.der $KEY_PATH
...
function client {
  ...
  echo "> We will use src/mesh_key.der if it already exists, or we can try and fetch it..."
  ...
  if [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]]; then
    ...
    scp $server_user@$server_ip:/opt/container-data/mesh/mesh_com/modules/sc-mesh-secure-deployment/src/mesh_key.der $MESH_COM_ROOT$KEY_PATH
    pkcs11-tool --module $LIB -l --pin 1234 --write-object root_cert.der --type pubkey --id 2222  --label root
  fi
