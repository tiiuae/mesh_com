...
function server {
  ...
  if [ $execution_ctx = "br_hardened" ]; then
    ...
    if [ "$2" == "-t" ] ; then
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der" --test
    else
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der"
    fi
  else
    pushd .
    cd ../..
    ...
    # Install the server and provision the certificate generated by cryptolib
    if [ "$2" == "-t" ] ; then
        sudo python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der" --test
    else
        sudo python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der"
    fi
  fi
}
...
function client {
  ...
  if [ "$2" == "-t" ] ; then
    python3 $MESH_COM_ROOT$CLIENT_SRC_PATH -c $CERTS_PATH"mesh_cert.der" -s http://$server_ip:5000 --test
  else
    python3 $MESH_COM_ROOT$CLIENT_SRC_PATH -c $CERTS_PATH"mesh_cert.der" -s http://$server_ip:5000
  fi
}


function managekeys {
  python3 src/primitives.py # to clean
  CERTS_PATH=/etc/ssl/certs/
  part1=$(dirname "$(pwd)")
