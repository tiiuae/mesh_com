...
function server {
  ...
  if [ $execution_ctx = "br_hardened" ]; then
    ...
    if [ "$2" == "-t" ] ; then
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der" --test -m $mesh_mode
    else
        python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der" -m $mesh_mode
    fi
  else
    pushd .
    cd ../..
...
function client {
  ...
  if [[ "$2" == "-ci" ]]; then
    echo "We need to be connect to the same network as the server... Connect to an Access Point? (Y/N): N"
  else
