...
#-----------------------------------------------------------------------------#
# This function now can receive IP mesh address as parameter
function server {
  ...
  if [ -z "$2" ] #checking if we receive a parameter, otherwise run with default value
  then
    sudo python3 src/server-mesh.py -c /etc/ssl/certs/ecc_key.der
  else
    sudo python3 src/server-mesh.py -c /etc/ssl/certs/ecc_key.der $2
  fi
}
...
function client {
  ...
  if [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]]; then
    ...
    scp $server_user@$server_ip:/home/$server_user/mesh_com/modules/sc-mesh-secure-deployment/src/ecc_key.der /etc/ssl/certs/ecc_key.der
