...
function server {
  ...
  if [ $execution_ctx = "br_hardened" ]; then
    ...
    # Install the server and provision the certificate generated by cryptolib
    if [ "$2" == "-t" ] ; then
        sudo python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der" --test $mesh_mode
    else
        sudo python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $CERTS_PATH"mesh_cert.der" $mesh_mode
    fi
  fi
}
