...
function server {
  ...
  if [ $execution_ctx = "br_hardened" ]; then
    ...
    if [ "$#" = 1 ]; then
      ...
      sudo python3 $MESH_COM_ROOT$SERVER_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH -t
    fi
  else
    pushd .
    cd ../..
...
function client {
  ...
  if [ "$#" = 1 ]; then
    ...
    python3 $MESH_COM_ROOT$CLIENT_SRC_PATH -c $MESH_COM_ROOT$KEY_PATH -s http://$server_ip:5000 -t
  fi
}
