...
EDDSA_ROOT_CA_CRT="$MSPKI/eddsa/root.crt"
RSA_ROOT_CA_CRT="$MSPKI/rsa/root.crt"
ECDSA_ROOT_CA_KEY="$MSPKI/ecdsa/root.key"
EDDSA_ROOT_CA_KEY="$MSPKI/eddsa/root.key"
RSA_ROOT_CA_KEY="$MSPKI/rsa/root.key"
...
root_signs_csr()
{
	...
	CRT="$2"
	KEYPAIR_TYPE="$CURRENT_KEYPAIR_TYPE"
        #TEMPROOT_KEY="${UPPERCASE_KEYPAIR_TYPE}_ROOT_CA_KEY"
        #TEMPROOT_CRT="${UPPERCASE_KEYPAIR_TYPE}_ROOT_CA_CRT"
	...
	#TEMPSOCA_KEY="TEMPORARY_${UPPERCASE_KEYPAIR_TYPE}_SECURITY_OFFICER_CA_KEY"
        ...
        openssl x509 -in "$CSR" -out "$CRT" -sha256 -days 90 -req -set_serial "0x$SERIAL" -CA "${!TEMPROOT_CRT}" -CAkey "${!TEMPROOT_KEY}" -extensions default.extensions -extfile <( cat <<- EOF
		[ default ]
		...
		#po$MSPKI$MSPKIlicyIdentifier       = 1.3.6.1.4.1.53429
...
sign_bundle()
{
	...
	#openssl pkeyutl -sign -inkey "${!TEMPROOT_KEY}"  -out "$BUNDLE.$KEYPAIR_TYPE-sig" -rawin -in "$BUNDLE"
	openssl pkeyutl -sign -inkey "${!TEMPROOT_KEY}" -out "$BUNDLE.$KEYPAIR_TYPE-sig" -rawin -in "$BUNDLE" || openssl dgst -sha256 -sign "${!TEMPROOT_KEY}" -out "$BUNDLE.securityofficer-$KEYPAIR_TYPE-sig" "$BUNDLE" # lacks Ed25519 support
}



# Each of the IDs in the functions above end up as a SubjectAltName (or list
# thereof if we later decide to not optimize as much as possible) but they all
# share the same SubjectName as the one thing they have in common. Choosing the
# distinguished name to stick in the SN field can probably still be improved,
# for now let's see what we can do with the HSM serial number (if we have a
# real HSM!), the CPU serial number (probably different format between Arm and
# RISC-V) or as a last recourse something we manage ourselves (hopefully not).
SUBJECT_NAME=`generate_subject_name`

# FIXME: format it as a Distinguished Name?
COMMON_NAME="$SUBJECT_NAME"

for KEYPAIR_TYPE in ecdsa eddsa rsa; do
