...
function command_exists {
  for cmd in $1; do
    ...
    mapfile -t INSTALLED < <(apt -qq list "$cmd" 2>/dev/null | awk -F[ '{print $2}')
...
function default() {
  ...
  then
    ...
    read -r -p "- Custom IP (e.g. XX.0.0.1): " ip
  else
      if [[ "$1" == "AuthAP"* ]];
        then
          ssid="$1"
...
if [ -z "$2" ] # interface
  then
    ...
    mapfile -t interfaces_arr < <(ip link | awk -F: '$0 !~ "lo|vir|doc|eth|bat|^[^0-9]"{print $2}')
