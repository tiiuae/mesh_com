...
def main():
    ...
    while True:
        if args.low_latency:
            ...
            channel_quality, prob = estimator.estimate(feat_array)
            # Print channel quality and frequencies
            print_channel_quality(args, channel_quality, prob, frequencies)
        else:
            channel_quality = np.zeros(1)

        # If channel quality is below the threshold
        if channel_quality.item() < args.threshold:
