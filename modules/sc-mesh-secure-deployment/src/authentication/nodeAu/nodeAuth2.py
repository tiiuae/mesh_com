...
if __name__ == "__main__":
    ...
    if level >= 3:
        ...
        try:
            client(nodeID, addr[0], my_mac_mesh)  # send my mac
        except ConnectionRefusedError:
            time.sleep(7)
            client(nodeID, addr[0], my_mac_mesh)
        client_mesh_ip, _ = server_auth(myID)
        try:
            client(nodeID, addr[0], my_mesh_ip)  # send my mesh ip
        except ConnectionRefusedError:
            time.sleep(7)
            client(nodeID, addr[0], my_mesh_ip)
        info = {'ID': 'provServer', 'MAC': client_mac, 'IP': client_mesh_ip, 'PubKey_fpr': client_fpr[0], 'trust_level': level}
